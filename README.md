...
- **複数 Slave ID**: 左ペインで ID を切り替え、各スレーブに機器名（タグ）を付与可能
...
どちらを編集しても内部の Raw 値に反映され、相手列が自動更新されます。Decoded は `0x` 付き16進数で入力してください。

## デフォルト

- Slave ID `1` が最初から登録されています
...
2. 「スレーブ」タブで設定値を表に追加
3. 右上「TCP 開始」で待受
4. 外部クライアントから Read/Write → 「通信ログ」タブでパケット確認、Slave 一覧の丸が緑に変化
