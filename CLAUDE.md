...
**Scenario runner** (`modbus_sim/scenario.py`, "シナリオ" tab, `POST /api/scenario/run`): a JSON list of steps (`set_point` / `set_word_order` / `set_frame_fault` / `start_server` / `stop_server` / `master_connect` / `master_request` with `expect` asserts / `wait` / `log`) run in order on an `AppState`, returning per-step pass/fail. Example in `examples/scenario_example.json`.
